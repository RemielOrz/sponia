...
function Swipe(container, options) {
  ...
  if(options.axis == 'y') container.className += ' swipe-y';
  ...
  function slide(to, slideSpeed) {
    ...
    if (browser.transitions) {
      ...
      if (options.continuous){
        if(options.axis == 'y') {
          move(circle(to - direction), -(height * direction), 0); // we need to get the next in place
        }else{
  ...
  var events = {
    ...
    move: function(event) {
      ...
      var isValidSlide,isPastBounds,direction;
      if(options.axis == 'y') {
        // determine if slide attempt triggers next/prev slide
        isValidSlide =
            Number(duration) < 250               // if slide duration is less than 250ms
            && Math.abs(delta.y) > 20            // and if slide amt is greater than 20px
            || Math.abs(delta.y) > width/2;      // or if slide amt is greater than half the width
